...
func (m MsgDelegate) ValidateBasic() error {
	if !m.Amount.Amount.GT(sdk.ZeroInt()) {
		return status.Errorf(codes.InvalidArgument, "Kaiju delegation amount must be more than zero")
	}
	return nil
}
...
func (m MsgRedelegate) ValidateBasic() error {
	if m.Amount.Amount.LTE(sdk.ZeroInt()) {
		return status.Errorf(codes.InvalidArgument, "Kaiju redelegation amount must be more than zero")
	}
	return nil
}
...
func (m MsgUndelegate) ValidateBasic() error {
	if m.Amount.Amount.LTE(sdk.ZeroInt()) {
		return status.Errorf(codes.InvalidArgument, "Kaiju undelegate amount must be more than zero")
	}
	return nil
}
