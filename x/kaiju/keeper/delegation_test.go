...
import (
	...
	"github.com/furya-official/kaiju/x/kaiju"
...
var KAIJU_TOKEN_DENOM = "kaiju"
var KAIJU_2_TOKEN_DENOM = "kaiju2"
...
func TestDelegationWithASingleAsset(t *testing.T) {
	...
	app.KaijuKeeper.InitGenesis(ctx, &types.GenesisState{
		...
		Assets: []types.KaijuAsset{
			types.NewKaijuAsset(KAIJU_TOKEN_DENOM, sdk.NewDec(2), sdk.NewDec(0), genesisTime),
			types.NewKaijuAsset(KAIJU_2_TOKEN_DENOM, sdk.NewDec(10), sdk.NewDec(0), genesisTime),
		},
	})
	...
	val, err := app.KaijuKeeper.GetKaijuValidator(ctx, valAddr)
	require.NoError(t, err)

	// Mint kaiju tokens
	err = app.BankKeeper.MintCoins(ctx, minttypes.ModuleName, sdk.NewCoins(sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(2000_000))))
	require.NoError(t, err)
	err = app.BankKeeper.MintCoins(ctx, minttypes.ModuleName, sdk.NewCoins(sdk.NewCoin(KAIJU_2_TOKEN_DENOM, sdk.NewInt(2000_000))))
	require.NoError(t, err)
	err = app.BankKeeper.SendCoinsFromModuleToAccount(ctx, minttypes.ModuleName, delAddr, sdk.NewCoins(sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(2000_000))))
	require.NoError(t, err)
	err = app.BankKeeper.SendCoinsFromModuleToAccount(ctx, minttypes.ModuleName, delAddr, sdk.NewCoins(sdk.NewCoin(KAIJU_2_TOKEN_DENOM, sdk.NewInt(2000_000))))
	require.NoError(t, err)
	...
	// Delegate
	_, err = app.KaijuKeeper.Delegate(ctx, delAddr, val, sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(1000_000)))
	require.NoError(t, err)

	// Manually trigger rebalancing
	app.KaijuKeeper.GetAssetByDenom(ctx, KAIJU_TOKEN_DENOM)
	assets := app.KaijuKeeper.GetAllAssets(ctx)
	err = app.KaijuKeeper.RebalanceBondTokenWeights(ctx, assets)
	require.NoError(t, err)

	kaijuBonded := app.StakingKeeper.GetDelegatorBonded(ctx, moduleAddr)
	// Total KAIJU tokens should be 2 * totalBonded
	require.Equal(t, totalBonded.Mul(sdk.NewInt(2)).String(), kaijuBonded.String())
	...
	// Check delegation in kaiju module
	kaijuDelegation, found := app.KaijuKeeper.GetDelegation(ctx, delAddr, val, KAIJU_TOKEN_DENOM)
	...
	// Check asset
	asset, _ := app.KaijuKeeper.GetAssetByDenom(ctx, KAIJU_TOKEN_DENOM)
	require.Equal(t, types.KaijuAsset{
		Denom:                KAIJU_TOKEN_DENOM,
...
func TestDelegationWithMultipleAssets(t *testing.T) {
	...
	app.KaijuKeeper.InitGenesis(ctx, &types.GenesisState{
		Params: types.DefaultParams(),
		Assets: []types.KaijuAsset{
			types.NewKaijuAsset(KAIJU_TOKEN_DENOM, sdk.NewDec(2), sdk.NewDec(0), ctx.BlockTime()),
			types.NewKaijuAsset(KAIJU_2_TOKEN_DENOM, sdk.NewDec(10), sdk.NewDec(0), ctx.BlockTime()),
		},
	})
	delegations := app.StakingKeeper.GetAllDelegations(ctx)
	require.Len(t, delegations, 1)
	...
	// Delegate
	_, err = app.KaijuKeeper.Delegate(ctx, delAddr, val, sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(2000_000)))
	require.NoError(t, err)
	// Delegate with another denom
	_, err = app.KaijuKeeper.Delegate(ctx, delAddr, val, sdk.NewCoin(KAIJU_2_TOKEN_DENOM, sdk.NewInt(1000_000)))
	require.NoError(t, err)

	// Manually trigger rebalancing
	app.KaijuKeeper.GetAssetByDenom(ctx, KAIJU_2_TOKEN_DENOM)
	assets := app.KaijuKeeper.GetAllAssets(ctx)
	err = app.KaijuKeeper.RebalanceBondTokenWeights(ctx, assets)
	require.NoError(t, err)
...
func TestDelegationWithUnknownAssets(t *testing.T) {
	...
	_, err = app.KaijuKeeper.Delegate(ctx, delAddr, val, sdk.NewCoin("UNKNOWN", sdk.NewInt(2000_000)))
	require.Error(t, err)
}
...
func TestSuccessfulRedelegation(t *testing.T) {
	...
	require.NoError(t, err)
	val1, err := app.KaijuKeeper.GetKaijuValidator(ctx, valAddr1)
	require.NoError(t, err)
	...
	err = app.BankKeeper.MintCoins(ctx, minttypes.ModuleName, sdk.NewCoins(sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(2000_000))))
	require.NoError(t, err)
	err = app.BankKeeper.SendCoinsFromModuleToAccount(ctx, minttypes.ModuleName, delAddr1, sdk.NewCoins(sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(1000_000))))
	require.NoError(t, err)
	err = app.BankKeeper.SendCoinsFromModuleToAccount(ctx, minttypes.ModuleName, delAddr2, sdk.NewCoins(sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(1000_000))))
	require.NoError(t, err)

	// First delegate to validator 1
	_, err = app.KaijuKeeper.Delegate(ctx, delAddr1, val1, sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(500_000)))
	require.NoError(t, err)

	// Then redelegate to validator2
	_, err = app.KaijuKeeper.Redelegate(ctx, delAddr1, val1, val2, sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(500_000)))
	require.NoError(t, err)

	_, stop := kaiju.RunAllInvariants(ctx, app.KaijuKeeper)
	...
	iter := app.KaijuKeeper.IterateRedelegationsByDelegator(ctx, delAddr1)
	defer iter.Close()
	...
	for ; iter.Valid(); iter.Next() {
		...
		require.Equal(t, types.Redelegation{
			...
			Balance:             sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(500_000)),
		}, redelegation)
	}

	// Check if the delegation objects are correct
	_, found := app.KaijuKeeper.GetDelegation(ctx, delAddr1, val1, KAIJU_TOKEN_DENOM)
	require.False(t, found)
	dstDelegation, found := app.KaijuKeeper.GetDelegation(ctx, delAddr1, val2, KAIJU_TOKEN_DENOM)
	require.Equal(t, types.Delegation{
		DelegatorAddress: delAddr1.String(),
	...
	iter = app.KaijuKeeper.IterateRedelegationsBySrcValidator(ctx, valAddr1)
	require.True(t, iter.Valid())

	// User then delegates to validator2
	_, err = app.KaijuKeeper.Delegate(ctx, delAddr2, val2, sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(500_000)))
	require.NoError(t, err)
	...
	_, err = app.KaijuKeeper.Redelegate(ctx, delAddr2, val2, val1, sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(500_000)))
	...
	deleted := app.KaijuKeeper.CompleteRedelegations(ctx)
	...
	iter = app.KaijuKeeper.IterateRedelegationsByDelegator(ctx, delAddr2)
	require.False(t, iter.Valid())
...
func TestRedelegationFailsWithTransitiveDelegation(t *testing.T) {
	...
	_, err = app.KaijuKeeper.Redelegate(ctx, delAddr1, val2, val1, sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(500_000)))
	require.Error(t, err)
}
...
func TestRedelegationFailsWithGreaterAmount(t *testing.T) {
	...
	_, err = app.KaijuKeeper.Redelegate(ctx, delAddr2, val2, val1, sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(1000_000)))
	require.Error(t, err)
}
...
func TestSuccessfulUndelegation(t *testing.T) {
	...
	coin := app.BankKeeper.GetBalance(ctx, delAddr, KAIJU_TOKEN_DENOM)
	require.Equal(t, sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(1000_000)), coin)
	...
	_, err = app.KaijuKeeper.Undelegate(ctx, delAddr, val, sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(250_000)))
	_, err = app.KaijuKeeper.Undelegate(ctx, delAddr, val, sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(250_000)))
	require.NoError(t, err)

	// Check if undelegations were stored correctly
	iter := app.KaijuKeeper.IterateUndelegationsByCompletionTime(ctx, ctx.BlockTime().Add(unbondingTime).Add(time.Second))
	require.True(t, iter.Valid())
	...
	require.Equal(t, types.QueuedUndelegation{Entries: []*types.Undelegation{
		{
			...
			Balance:          sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(250_000)),
		},
		{
			DelegatorAddress: delAddr.String(),
			ValidatorAddress: val.GetOperator().String(),
	...
	err = app.KaijuKeeper.CompleteUndelegations(ctx)
	require.NoError(t, err)
	...
	coin = app.BankKeeper.GetBalance(ctx, delAddr, KAIJU_TOKEN_DENOM)
	require.Equal(t, sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(1500_000)), coin)
...
func TestUndelegationWithoutDelegation(t *testing.T) {
	...
	_, err = app.KaijuKeeper.Undelegate(ctx, delAddr, val, sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(1000_000)))
	require.Error(t, err)
}
...
func TestUndelegateAfterClaimingTakeRate(t *testing.T) {
	...
	app.KaijuKeeper.InitGenesis(ctx, &types.GenesisState{
		...
		Assets: []types.KaijuAsset{
			types.NewKaijuAsset(KAIJU_TOKEN_DENOM, sdk.NewDec(2), sdk.NewDec(0), ctx.BlockTime()),
			types.NewKaijuAsset(KAIJU_2_TOKEN_DENOM, sdk.NewDec(10), sdk.MustNewDecFromStr("0.5"), ctx.BlockTime()),
		},
	})
	...
	addrs := test_helpers.AddTestAddrsIncremental(app, ctx, 4, sdk.NewCoins(
		sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(1000_000_000)),
		sdk.NewCoin(KAIJU_2_TOKEN_DENOM, sdk.NewInt(2000_000_000)),
	))
	...
	_, err = app.KaijuKeeper.Delegate(ctx, user1, val1, sdk.NewCoin(KAIJU_2_TOKEN_DENOM, sdk.NewInt(1000_000_000)))
	require.NoError(t, err)
	_, err = app.KaijuKeeper.Delegate(ctx, user2, val2, sdk.NewCoin(KAIJU_2_TOKEN_DENOM, sdk.NewInt(1000_000_000)))
	require.NoError(t, err)
	...
	coins, err := app.KaijuKeeper.DeductAssetsHook(ctx, assets)
	require.NoError(t, err)
	require.False(t, coins.IsZero())

	res, err := queryServer.KaijuDelegation(ctx, &types.QueryKaijuDelegationRequest{
	...
	_, err = app.KaijuKeeper.Undelegate(ctx, user1, val1, sdk.NewCoin(KAIJU_2_TOKEN_DENOM, sdk.NewInt(1000_000_000)))
	require.Error(t, err)

	// Undelegate token with current amount should pass
	_, err = app.KaijuKeeper.Undelegate(ctx, user1, val1, sdk.NewCoin(KAIJU_2_TOKEN_DENOM, del.Balance.Amount))
	require.NoError(t, err)

	// User should have everything withdrawn
	_, found := app.KaijuKeeper.GetDelegation(ctx, user1, val1, KAIJU_2_TOKEN_DENOM)
	require.False(t, found)

	// Delegate again
	_, err = app.KaijuKeeper.Delegate(ctx, user1, val1, sdk.NewCoin(KAIJU_2_TOKEN_DENOM, sdk.NewInt(500_000_000)))
	require.NoError(t, err)

	ctx = ctx.WithBlockTime(ctx.BlockTime().Add(time.Minute * 1)).WithBlockHeight(2)

	_, err = app.KaijuKeeper.Delegate(ctx, user1, val1, sdk.NewCoin(KAIJU_2_TOKEN_DENOM, sdk.NewInt(400_000_000)))
	require.NoError(t, err)
...
func TestDelegationWithNativeStakingChanges(t *testing.T) {
	...
	// Stake some kaiju tokens
	_, err = app.KaijuKeeper.Delegate(ctx, user1, val1, sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(2000_000)))
	require.NoError(t, err)
	_, err = app.KaijuKeeper.Delegate(ctx, user2, val2, sdk.NewCoin(KAIJU_2_TOKEN_DENOM, sdk.NewInt(2000_000)))
	require.NoError(t, err)
	assets := app.KaijuKeeper.GetAllAssets(ctx)
	err = app.KaijuKeeper.RebalanceBondTokenWeights(ctx, assets)
	require.NoError(t, err)
