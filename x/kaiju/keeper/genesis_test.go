...
func TestGenesis(t *testing.T) {
	...
	app.KaijuKeeper.InitGenesis(ctx, &types.GenesisState{
		...
		Assets: []types.KaijuAsset{
			types.NewKaijuAsset("stake", sdk.NewDec(1), sdk.ZeroDec(), ctx.BlockTime()),
		},
	})

	delay := app.KaijuKeeper.RewardDelayTime(ctx)
	require.Equal(t, time.Duration(1000000), delay)

	interval := app.KaijuKeeper.RewardClaimInterval(ctx)
	require.Equal(t, time.Duration(1000000), interval)

	lastClaimTime := app.KaijuKeeper.LastRewardClaimTime(ctx)
...
func TestExportAndImportGenesis(t *testing.T) {
	...
	require.NoError(t, err)
	val1, err := app.KaijuKeeper.GetKaijuValidator(ctx, valAddr)
	require.NoError(t, err)
	...
	// Delegate
	delegationCoin := sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(1000_000_000))
	...
	require.NoError(t, err)
	_, err = app.KaijuKeeper.Delegate(ctx, delAddr, val1, delegationCoin)
	require.NoError(t, err)

	// Redelegate
	_, err = app.KaijuKeeper.Redelegate(ctx, delAddr, val1, val2, sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(500_000_000)))
	require.NoError(t, err)

	// Undelegate
	_, err = app.KaijuKeeper.Undelegate(ctx, delAddr, val1, sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(500_000_000)))
	require.NoError(t, err)

	// Trigger update asset
	ctx = ctx.WithBlockTime(ctx.BlockTime().Add(time.Hour * 25)).WithBlockHeight(ctx.BlockHeight() + 1)
	err = app.KaijuKeeper.UpdateKaijuAsset(ctx, types.NewKaijuAsset(KAIJU_TOKEN_DENOM, sdk.MustNewDecFromStr("0.5"), sdk.ZeroDec(), ctx.BlockTime()))
	require.NoError(t, err)

	genesisState := app.KaijuKeeper.ExportGenesis(ctx)
	...
	app.KaijuKeeper.InitGenesis(ctx, genesisState)

	// Check all items in the kaiju store match
