...
// Delegate is the entry point for delegators to delegate kaiju assets to validators
...
func (k Keeper) Delegate(ctx sdk.Context, delAddr sdk.AccAddress, validator types.KaijuValidator, coin sdk.Coin) (*sdk.Dec, error) {
	// Check if asset is whitelisted as an kaiju asset
	asset, found := k.GetAssetByDenom(ctx, coin.Denom)
	if !found {
		return nil, status.Errorf(codes.NotFound, "asset with denom: %s does not exist in kaiju whitelist", coin.Denom)
	}

	// Check and send delegated tokens into the kaiju module address
...
func (k Keeper) Redelegate(ctx sdk.Context, delAddr sdk.AccAddress, srcVal types.KaijuValidator, dstVal types.KaijuValidator, coin sdk.Coin) (*time.Time, error) {
...
func (k Keeper) Undelegate(ctx sdk.Context, delAddr sdk.AccAddress, validator types.KaijuValidator, coin sdk.Coin) (*time.Time, error) {
	asset, found := k.GetAssetByDenom(ctx, coin.Denom)
	if !found {
		return nil, status.Errorf(codes.NotFound, "Asset with denom: %s does not exist", coin.Denom)
	}
	...
	// Remove tokens and shares from the kaiju asset
...
func (k Keeper) GetDelegation(ctx sdk.Context, delAddr sdk.AccAddress, validator types.KaijuValidator, denom string) (d types.Delegation, found bool) {
...
func (k Keeper) SetValidator(ctx sdk.Context, val types.KaijuValidator) {
	store := ctx.KVStore(k.storeKey)
	key := types.GetKaijuValidatorInfoKey(val.GetOperator())
	vb := k.cdc.MustMarshal(val.KaijuValidatorInfo)
	store.Set(key, vb)
}

func (k Keeper) SetValidatorInfo(ctx sdk.Context, valAddr sdk.ValAddress, val types.KaijuValidatorInfo) {
	store := ctx.KVStore(k.storeKey)
...
func (k Keeper) ValidateDelegatedAmount(delegation types.Delegation, coin sdk.Coin, val types.KaijuValidator, asset types.KaijuAsset) (shares sdk.Dec, err error) {
...
func (k Keeper) upsertDelegationWithNewTokens(ctx sdk.Context, delAddr sdk.AccAddress, validator types.KaijuValidator, coin sdk.Coin, asset types.KaijuAsset) (types.Delegation, sdk.Dec) {
...
func (k Keeper) reduceDelegationShares(ctx sdk.Context, delAddr sdk.AccAddress, validator types.KaijuValidator, coin sdk.Coin, shares sdk.Dec, delegation types.Delegation) {
...
func (k Keeper) updateValidatorShares(ctx sdk.Context, validator types.KaijuValidator, delegationShares sdk.DecCoins, validatorShares sdk.DecCoins, isAdd bool) {
	if isAdd {
...
// GetKaijuBondedAmount returns the total amount of bonded native tokens that are not in the
// unbonding pool
func (k Keeper) GetKaijuBondedAmount(ctx sdk.Context, delegator sdk.AccAddress) math.Int {
	bonded := sdk.ZeroDec()
