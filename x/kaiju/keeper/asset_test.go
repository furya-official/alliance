...
func TestRebalancingAfterRewardsRateChange(t *testing.T) {
	...
	app.KaijuKeeper.InitGenesis(ctx, &types.GenesisState{
		...
		Assets: []types.KaijuAsset{
			types.NewKaijuAsset(KAIJU_TOKEN_DENOM, sdk.NewDec(2), sdk.ZeroDec(), startTime),
			types.NewKaijuAsset(KAIJU_2_TOKEN_DENOM, sdk.NewDec(10), sdk.ZeroDec(), startTime),
		},
	})
	...
	// Start by delegating
	_, err = app.KaijuKeeper.Delegate(ctx, user1, val1, sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(1000_000)))
	require.NoError(t, err)
	...
	// Expecting voting power for the kaiju module
	...
	err = app.KaijuKeeper.UpdateKaijuAsset(ctx, types.KaijuAsset{
	...
	iter := app.KaijuKeeper.IterateWeightChangeSnapshot(ctx, KAIJU_TOKEN_DENOM, val.GetOperator(), 0)
...
func TestRebalancingWithUnbondedValidator(t *testing.T) {
	...
	addrs := test_helpers.AddTestAddrsIncremental(app, ctx, 5, sdk.NewCoins(
		...
		sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(50_000_000)),
		sdk.NewCoin(KAIJU_2_TOKEN_DENOM, sdk.NewInt(50_000_000)),
	))
	pks := test_helpers.CreateTestPubKeys(2)
	...
	_, err = app.KaijuKeeper.Delegate(ctx, user1, val1, sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(20_000_000)))
	require.NoError(t, err)
	_, err = app.KaijuKeeper.Delegate(ctx, user1, val2, sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(10_000_000)))
	require.NoError(t, err)
	_, err = app.KaijuKeeper.Delegate(ctx, user2, val1, sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(10_000_000)))
	require.NoError(t, err)
	_, err = app.KaijuKeeper.Delegate(ctx, user2, val2, sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(10_000_000)))
	require.NoError(t, err)

	_, err = app.KaijuKeeper.Delegate(ctx, user1, val1, sdk.NewCoin(KAIJU_2_TOKEN_DENOM, sdk.NewInt(10_000_000)))
	require.NoError(t, err)
	_, err = app.KaijuKeeper.Delegate(ctx, user1, val2, sdk.NewCoin(KAIJU_2_TOKEN_DENOM, sdk.NewInt(10_000_000)))
	require.NoError(t, err)
	_, err = app.KaijuKeeper.Delegate(ctx, user2, val1, sdk.NewCoin(KAIJU_2_TOKEN_DENOM, sdk.NewInt(10_000_000)))
	require.NoError(t, err)
	_, err = app.KaijuKeeper.Delegate(ctx, user2, val2, sdk.NewCoin(KAIJU_2_TOKEN_DENOM, sdk.NewInt(10_000_000)))
	require.NoError(t, err)
	require.NoError(t, err)
	...
	val1, _ = app.KaijuKeeper.GetKaijuValidator(ctx, valAddr1)
	val2, _ = app.KaijuKeeper.GetKaijuValidator(ctx, valAddr2)
	...
	app.KaijuKeeper.GetKaijuValidator(ctx, valAddr2)
...
func TestRebalancingWithDelayedRewardsStartTime(t *testing.T) {
	...
	app.KaijuKeeper.InitGenesis(ctx, &types.GenesisState{
		Params: types.DefaultParams(),
		Assets: []types.KaijuAsset{
			types.NewKaijuAsset(KAIJU_TOKEN_DENOM, sdk.MustNewDecFromStr("0.5"), sdk.MustNewDecFromStr("0.1"), startTime.Add(time.Hour*24)),
			types.NewKaijuAsset(KAIJU_2_TOKEN_DENOM, sdk.MustNewDecFromStr("0.2"), sdk.MustNewDecFromStr("0.1"), startTime.Add(time.Hour*24*2)),
		},
	})
	...
	// Expect that rewards rates are updated only for kaiju 1
	...
	// Expect that rewards rates are updated all kaijus
...
func TestConsumingRebalancingEvent(t *testing.T) {
	...
	app.KaijuKeeper.QueueAssetRebalanceEvent(ctx)
	store := ctx.KVStore(app.KaijuKeeper.StoreKey())
	key := types.AssetRebalanceQueueKey
	b := store.Get(key)
	require.NotNil(t, b)

	require.True(t, app.KaijuKeeper.ConsumeAssetRebalanceEvent(ctx))
	b = store.Get(key)
	require.Nil(t, b)

	require.False(t, app.KaijuKeeper.ConsumeAssetRebalanceEvent(ctx))
}
...
func TestRewardWeightDecay(t *testing.T) {
	...
	app.KaijuKeeper.CreateKaiju(ctx, &types.MsgCreateKaijuProposal{
		Title:                "",
		Description:          "",
		Denom:                KAIJU_TOKEN_DENOM,
		RewardWeight:         sdk.NewDec(1),
	...
	app.KaijuKeeper.RewardWeightChangeHook(ctx, assets)
	...
	app.KaijuKeeper.RewardWeightChangeHook(ctx, assets)
	updatedAsset, _ := app.KaijuKeeper.GetAssetByDenom(ctx, KAIJU_TOKEN_DENOM)
	...
	// Updating the kaiju asset through proposal should queue another decay event
	app.KaijuKeeper.UpdateKaiju(ctx, &types.MsgUpdateKaijuProposal{
		Title:                "",
		Description:          "",
		Denom:                KAIJU_TOKEN_DENOM,
		RewardWeight:         sdk.MustNewDecFromStr("0.5"),
	...
	// Updating kaiju asset again with a non-zero decay
	app.KaijuKeeper.UpdateKaiju(ctx, &types.MsgUpdateKaijuProposal{
		Title:                "",
		Description:          "",
		Denom:                KAIJU_TOKEN_DENOM,
	...
	err = app.KaijuKeeper.CreateKaiju(ctx, &types.MsgCreateKaijuProposal{
		Title:                "",
		Description:          "",
		Denom:                KAIJU_2_TOKEN_DENOM,
		RewardWeight:         sdk.NewDec(1),
	...
	// Updating kaiju asset again with a non-zero decay
	err = app.KaijuKeeper.UpdateKaiju(ctx, &types.MsgUpdateKaijuProposal{
		Title:                "",
		Description:          "",
		Denom:                KAIJU_2_TOKEN_DENOM,
		RewardWeight:         sdk.MustNewDecFromStr("0.5"),
...
func TestRewardWeightDecayOverTime(t *testing.T) {
	...
	addrs := test_helpers.AddTestAddrsIncremental(app, ctx, 5, sdk.NewCoins(
		...
		sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(5_000_000)),
		sdk.NewCoin(KAIJU_2_TOKEN_DENOM, sdk.NewInt(5_000_000)),
	))
	...
	val0, _ := app.KaijuKeeper.GetKaijuValidator(ctx, _val0.GetOperator())
	require.NoError(t, err)
	...
	_, err = app.KaijuKeeper.Delegate(ctx, addrs[1], val0, sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(5_000_000)))
	require.NoError(t, err)
	//
	assets := app.KaijuKeeper.GetAllAssets(ctx)
	err = app.KaijuKeeper.RebalanceHook(ctx, assets)
	require.NoError(t, err)
	...
	totalDecayTime := (time.Hour * 24 * 10) - app.KaijuKeeper.RewardDelayTime(ctx)
	intervals := uint64(totalDecayTime / decayInterval)
	asset, _ = app.KaijuKeeper.GetAssetByDenom(ctx, KAIJU_TOKEN_DENOM)
	require.Equal(t, startTime.Add(app.KaijuKeeper.RewardDelayTime(ctx)).Add(decayInterval*time.Duration(intervals)), asset.LastRewardChangeTime)
...
func TestClaimTakeRate(t *testing.T) {
	...
	app.KaijuKeeper.InitGenesis(ctx, &types.GenesisState{
		...
		Assets: []types.KaijuAsset{
			types.NewKaijuAsset(KAIJU_TOKEN_DENOM, sdk.NewDec(2), sdk.MustNewDecFromStr("0.5"), startTime),
			types.NewKaijuAsset(KAIJU_2_TOKEN_DENOM, sdk.NewDec(10), sdk.NewDec(0), startTime),
		},
	})
	...
	addrs := test_helpers.AddTestAddrsIncremental(app, ctx, 1, sdk.NewCoins(
		...
		sdk.NewCoin(KAIJU_2_TOKEN_DENOM, sdk.NewInt(1000_000_000)),
	))
	user1 := addrs[0]

	app.KaijuKeeper.Delegate(ctx, user1, val1, sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(1000_000_000)))
	app.KaijuKeeper.Delegate(ctx, user1, val1, sdk.NewCoin(KAIJU_2_TOKEN_DENOM, sdk.NewInt(1000_000_000)))
	...
	coinsClaimed, _ := app.KaijuKeeper.DeductAssetsHook(ctx, assets)
	...
	require.Equal(t, expectedAmount.TruncateInt(), coins.AmountOf(KAIJU_TOKEN_DENOM))

	lastUpdate := app.KaijuKeeper.LastRewardClaimTime(ctx)
	require.Equal(t, startTime.Add(takeRateInterval), lastUpdate)

	asset, found := app.KaijuKeeper.GetAssetByDenom(ctx, KAIJU_TOKEN_DENOM)
	require.True(t, found)
	...
	require.Equal(t,
		sdk.NewDecFromInt(coinsClaimed.AmountOf(KAIJU_TOKEN_DENOM)),
		rewards.AmountOf(KAIJU_TOKEN_DENOM).Add(community.AmountOf(KAIJU_TOKEN_DENOM)),
	)
}
...
func TestClaimTakeRateToZero(t *testing.T) {
	...
	ctx = ctx.WithBlockHeight(1)
	takeRateInterval := time.Minute * 5
	asset := types.NewKaijuAsset(KAIJU_TOKEN_DENOM, sdk.NewDec(2), sdk.MustNewDecFromStr("0.8"), startTime)
	app.KaijuKeeper.InitGenesis(ctx, &types.GenesisState{
		Params: types.Params{
			RewardDelayTime:       time.Minute * 60,
			TakeRateClaimInterval: takeRateInterval,
	...
	for i := 0; i < 1000; i++ {
		...
		_, err = app.KaijuKeeper.DeductAssetsHook(ctx, assets)
		require.NoError(t, err)
	}
