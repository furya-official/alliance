...
// UpdateKaijuAsset updates the kaiju asset with new params
...
func (k Keeper) UpdateKaijuAsset(ctx sdk.Context, newAsset types.KaijuAsset) error {
	...
	if !newAsset.RewardWeight.Equal(asset.RewardWeight) {
		k.IterateKaijuValidatorInfo(ctx, func(valAddr sdk.ValAddress, info types.KaijuValidatorInfo) bool {
...
func (k Keeper) RebalanceHook(ctx sdk.Context, assets []*types.KaijuAsset) error {
...
// the difference.
func (k Keeper) RebalanceBondTokenWeights(ctx sdk.Context, assets []*types.KaijuAsset) (err error) {
	moduleAddr := k.accountKeeper.GetModuleAddress(types.ModuleName)
	kaijuBondAmount := k.GetKaijuBondedAmount(ctx, moduleAddr)

	nativeBondAmount := k.stakingKeeper.TotalBondedTokens(ctx).Sub(kaijuBondAmount)
	bondDenom := k.stakingKeeper.BondDenom(ctx)

	unbondedValidatorShares := sdk.NewDecCoins()
	var bondedValidators []types.KaijuValidator

	// Iterate through all kaiju validators to remove those that are unbonded.
	// Unbonded validators will be ignored when rebalancing.
	k.IterateKaijuValidatorInfo(ctx, func(valAddr sdk.ValAddress, info types.KaijuValidatorInfo) bool {
		var validator types.KaijuValidator
		validator, err = k.GetKaijuValidator(ctx, valAddr)
		if err != nil {
			return true
		}
...
func (k Keeper) SetAsset(ctx sdk.Context, asset types.KaijuAsset) {
	store := ctx.KVStore(k.storeKey)
	b := k.cdc.MustMarshal(&asset)
	store.Set(types.GetAssetKey(asset.Denom), b)
}

func (k Keeper) GetAllAssets(ctx sdk.Context) (assets []*types.KaijuAsset) {
	store := ctx.KVStore(k.storeKey)
...
func (k Keeper) GetAssetByDenom(ctx sdk.Context, denom string) (asset types.KaijuAsset, found bool) {
	store := ctx.KVStore(k.storeKey)
...
// DeductAssetsHook is called periodically to deduct from an kaiju asset (calculated by take_rate).
// The interval in which assets are deducted is set in module params
func (k Keeper) DeductAssetsHook(ctx sdk.Context, assets []*types.KaijuAsset) (sdk.Coins, error) {
...
// DeductAssetsWithTakeRate Deducts an kaiju asset using the take_rate
// The deducted asset is distributed to the fee_collector module account to be redistributed to stakers
func (k Keeper) DeductAssetsWithTakeRate(ctx sdk.Context, lastClaim time.Time, assets []*types.KaijuAsset) (sdk.Coins, error) {
...
func (k Keeper) SetRewardWeightChangeSnapshot(ctx sdk.Context, asset types.KaijuAsset, val types.KaijuValidator) {
...
func (k Keeper) RewardWeightChangeHook(ctx sdk.Context, assets []*types.KaijuAsset) {
	for _, asset := range assets {
