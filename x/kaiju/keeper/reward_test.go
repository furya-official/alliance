...
func TestRewardPoolAndGlobalIndex(t *testing.T) {
	...
	err = app.KaijuKeeper.AddAssetsToRewardPool(ctx, mintPoolAddr, val1, sdk.NewCoins(sdk.NewCoin("stake", sdk.NewInt(2000_000))))
	require.Error(t, err)
	...
	_, err = app.KaijuKeeper.Delegate(ctx, user2, val1, sdk.NewCoin(KAIJU_2_TOKEN_DENOM, sdk.NewInt(1000_000)))
	require.NoError(t, err)
	assets = app.KaijuKeeper.GetAllAssets(ctx)
	err = app.KaijuKeeper.RebalanceBondTokenWeights(ctx, assets)
	require.NoError(t, err)
	...
	err = app.KaijuKeeper.AddAssetsToRewardPool(ctx, mintPoolAddr, val1, sdk.NewCoins(sdk.NewCoin("stake2", sdk.NewInt(4000_000))))
	require.NoError(t, err)
...
func TestClaimRewards(t *testing.T) {
	...
	asset, _ = app.KaijuKeeper.GetAssetByDenom(ctx, KAIJU_2_TOKEN_DENOM)
	require.Equal(t,
		sdk.NewInt(1000_000),
		val1.TotalTokensWithAsset(asset).TruncateInt(),
	)
	...
	coins, err = app.KaijuKeeper.ClaimDelegationRewards(ctx, user1, val1, KAIJU_TOKEN_DENOM)
	require.NoError(t, err)
	...
	_, err = app.KaijuKeeper.ClaimDelegationRewards(ctx, user2, val1, KAIJU_TOKEN_DENOM)
	require.Error(t, err)

	// User 2 claims rewards
	coins, err = app.KaijuKeeper.ClaimDelegationRewards(ctx, user2, val1, KAIJU_2_TOKEN_DENOM)
	require.NoError(t, err)
	...
	delegation, found := app.KaijuKeeper.GetDelegation(ctx, user1, val1, KAIJU_TOKEN_DENOM)
	require.True(t, found)
	require.Equal(t, indices, types.NewRewardHistories(delegation.RewardHistory))

	delegation, found = app.KaijuKeeper.GetDelegation(ctx, user2, val1, KAIJU_2_TOKEN_DENOM)
	require.True(t, found)
	require.Equal(t, indices, types.NewRewardHistories(delegation.RewardHistory))
}
...
func TestClaimRewardsWithMultipleValidators(t *testing.T) {
	...
	app.KaijuKeeper.InitGenesis(ctx, &types.GenesisState{
		Params: types.DefaultParams(),
		Assets: []types.KaijuAsset{
			types.NewKaijuAsset(KAIJU_TOKEN_DENOM, sdk.NewDec(2), sdk.NewDec(0), startTime),
	...
	_, err = app.KaijuKeeper.Delegate(ctx, user2, val2, sdk.NewCoin(KAIJU_2_TOKEN_DENOM, sdk.NewInt(1000_000)))
	require.NoError(t, err)
	...
	coins, err = app.KaijuKeeper.ClaimDelegationRewards(ctx, user2, val2, KAIJU_2_TOKEN_DENOM)
	require.NoError(t, err)
...
func TestClaimRewardsAfterRewardsRatesChange(t *testing.T) {
	...
	addrs := test_helpers.AddTestAddrsIncremental(app, ctx, 4, sdk.NewCoins(
		sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(10_000_000)),
		sdk.NewCoin(KAIJU_2_TOKEN_DENOM, sdk.NewInt(10_000_000)),
	))
	...
	// New delegations
	app.KaijuKeeper.Delegate(ctx, user1, val1, sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(1000_000)))
	app.KaijuKeeper.Delegate(ctx, user2, val2, sdk.NewCoin(KAIJU_2_TOKEN_DENOM, sdk.NewInt(1000_000)))
	assets := app.KaijuKeeper.GetAllAssets(ctx)
	err = app.KaijuKeeper.RebalanceBondTokenWeights(ctx, assets)
	require.NoError(t, err)
	...
	err = app.KaijuKeeper.UpdateKaijuAsset(ctx, types.NewKaijuAsset(KAIJU_TOKEN_DENOM, sdk.NewDec(10), sdk.NewDec(0), ctx.BlockTime()))
	require.NoError(t, err)
	assets = app.KaijuKeeper.GetAllAssets(ctx)
	err = app.KaijuKeeper.RebalanceBondTokenWeights(ctx, assets)
	require.NoError(t, err)
	...
	val1, _ = app.KaijuKeeper.GetKaijuValidator(ctx, valAddr1)
	iter := app.KaijuKeeper.IterateWeightChangeSnapshot(ctx, KAIJU_TOKEN_DENOM, valAddr1, 0)
	...
	rewards1, err := app.KaijuKeeper.ClaimDelegationRewards(ctx, user1, val1, KAIJU_TOKEN_DENOM)
	require.NoError(t, err)
	require.Equal(t, sdk.NewInt(5_000_000+1_666_666), rewards1.AmountOf(bondDenom))

	rewards2, err := app.KaijuKeeper.ClaimDelegationRewards(ctx, user2, val2, KAIJU_2_TOKEN_DENOM)
	require.NoError(t, err)
...
func TestRewardClaimingAfterRatesDecay(t *testing.T) {
	...
	rewardStartDelay := app.KaijuKeeper.RewardDelayTime(ctx)
	...
	_, err = app.KaijuKeeper.Delegate(ctx, addrs[1], val0, sdk.NewCoin(KAIJU_2_TOKEN_DENOM, sdk.NewInt(5_000_000)))
	require.NoError(t, err)
	//
	assets := app.KaijuKeeper.GetAllAssets(ctx)
	...
	app.KaijuKeeper.AddAssetsToRewardPool(ctx, addrs[0], val0, sdk.NewCoins(sdk.NewCoin(bondDenom, sdk.NewInt(1000_000))))
	assets = app.KaijuKeeper.GetAllAssets(ctx)
	...
	coins, err := app.KaijuKeeper.ClaimDelegationRewards(ctx, addrs[1], val0, KAIJU_TOKEN_DENOM)
	coins2, err := app.KaijuKeeper.ClaimDelegationRewards(ctx, addrs[1], val0, KAIJU_2_TOKEN_DENOM)
