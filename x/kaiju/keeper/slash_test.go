...
func TestSlashingEvent(t *testing.T) {
	...
	addrs := test_helpers.AddTestAddrsIncremental(app, ctx, 4, sdk.NewCoins(
		sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(20_000_000)),
		sdk.NewCoin(KAIJU_2_TOKEN_DENOM, sdk.NewInt(20_000_000)),
	))
	...
	_, err = app.KaijuKeeper.Delegate(ctx, user1, val1, sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(10_000_000)))
	require.NoError(t, err)
	...
	asset1, _ := app.KaijuKeeper.GetAssetByDenom(ctx, KAIJU_TOKEN_DENOM)
	tokens := val1.TotalTokensWithAsset(asset1).TruncateInt()
	require.Equal(t, sdk.NewInt(20_000_000), tokens)
	asset2, _ := app.KaijuKeeper.GetAssetByDenom(ctx, KAIJU_2_TOKEN_DENOM)
...
func TestSlashingAfterRedelegation(t *testing.T) {
	...
	_, err = app.KaijuKeeper.Redelegate(ctx, user1, val1, val2, sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(10_000_000)))
	require.NoError(t, err)
	...
	delegation, _ := app.KaijuKeeper.GetDelegation(ctx, user1, val2, KAIJU_TOKEN_DENOM)
	...
	// Now we slash val 1
	app.KaijuKeeper.GetKaijuValidator(ctx, valAddr1)
	app.SlashingKeeper.Slash(ctx, valConAddr1, slashFraction, valPower1, 1)
...
func TestSlashingAfterUndelegation(t *testing.T) {
	...
	_, err = app.KaijuKeeper.Undelegate(ctx, user1, val1, sdk.NewCoin(KAIJU_TOKEN_DENOM, sdk.NewInt(10_000_000)))
	require.NoError(t, err)

	// Expect to have undelegation index saved
	undelegationIndexIter := app.KaijuKeeper.IterateUndelegationsBySrcValidator(ctx, valAddr1)
	...
	undelegationsIter := app.KaijuKeeper.IterateUndelegationsByCompletionTime(ctx, ctx.BlockTime().Add(app.StakingKeeper.UnbondingTime(ctx)).Add(time.Second))
	...
	// Expect that delegation stayed the same
	undelegationsIter = app.KaijuKeeper.IterateUndelegationsByCompletionTime(ctx, ctx.BlockTime())
	require.True(t, undelegationsIter.Valid())
