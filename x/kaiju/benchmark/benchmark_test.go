...
import (
	...
	"github.com/furya-official/kaiju/x/kaiju/benchmark"
...
func TestRunBenchmarks(t *testing.T) {
	...
	for b := 0; b < NUM_OF_BLOCKS; b += 1 {
		...
		proposer, err := app.KaijuKeeper.GetKaijuValidator(ctx, proposerAddr)
		require.NoError(t, err)
		...
		app.KaijuKeeper.CompleteRedelegations(ctx)
		...
		res, stop := kaiju.RunAllInvariants(ctx, app.KaijuKeeper)
		if stop {
			panic(res)
		}
	}
	...
	state := app.KaijuKeeper.ExportGenesis(ctx)
	file, _ := os.Create("./benchmark_genesis.json")
	defer file.Close()
	file.Write(app.AppCodec().MustMarshalJSON(state))
}

func delegateOperation(ctx sdk.Context, app *test_helpers.App, r *rand.Rand, assets []types.KaijuAsset, vals []sdk.AccAddress, dels []sdk.AccAddress) {
	...
	app.KaijuKeeper.Delegate(ctx, delAddr, val, coins)
...
func redelegateOperation(ctx sdk.Context, app *test_helpers.App, r *rand.Rand, assets []types.KaijuAsset, vals []sdk.AccAddress, dels []sdk.AccAddress) {
	...
	srcValidator, _ := app.KaijuKeeper.GetKaijuValidator(ctx, srcValAddr)
	asset, _ := app.KaijuKeeper.GetAssetByDenom(ctx, delegation.Denom)

	if app.KaijuKeeper.HasRedelegation(ctx, delAddr, srcValAddr, asset.Denom) {
		return
	}
	...
	dstValidator, _ := app.KaijuKeeper.GetKaijuValidator(ctx, dstValAddr)

	delegation, found := app.KaijuKeeper.GetDelegation(ctx, delAddr, srcValidator, asset.Denom)
	if !found {
		return
	}
	...
	_, err := app.KaijuKeeper.Redelegate(ctx, delAddr, srcValidator, dstValidator, sdk.NewCoin(delegation.Denom, amountToRedelegate))
	if err != nil {
		panic(err)
	}
}
...
func undelegateOperation(ctx sdk.Context, app *test_helpers.App, r *rand.Rand) {
	...
	validator, _ := app.KaijuKeeper.GetKaijuValidator(ctx, valAddr)
	asset, _ := app.KaijuKeeper.GetAssetByDenom(ctx, delegation.Denom)

	delegation, found := app.KaijuKeeper.GetDelegation(ctx, delAddr, validator, asset.Denom)
	if !found {
		return
	}
	...
	app.KaijuKeeper.Undelegate(ctx, delAddr, validator, sdk.NewCoin(asset.Denom, amountToUndelegate))
}
...
func claimRewardsOperation(ctx sdk.Context, app *test_helpers.App, r *rand.Rand) {
	...
	delegation, found := app.KaijuKeeper.GetDelegation(ctx, delAddr, validator, delegation.Denom)
	if !found {
		return
	}

	_, err := app.KaijuKeeper.ClaimDelegationRewards(ctx, delAddr, validator, delegation.Denom)
	if err != nil {
		panic(err)
	}
}
