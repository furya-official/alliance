...
func ValidateGenesis(data *types.GenesisState) error {
	...
	if len(data.Delegations) > 0 && len(data.Assets) == 0 {
		return types.ErrInvalidGenesisState.Wrap("cannot have delegations without kaiju assets")
	}
	if len(data.Delegations) > 0 && len(data.ValidatorInfos) == 0 {
		return types.ErrInvalidGenesisState.Wrap("cannot have delegations without kaiju validator infos")
	}
