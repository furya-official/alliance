...
import (
	...
	"github.com/furya-official/furya/x/furya/benchmark"
...
func TestRunBenchmarks(t *testing.T) {
	...
	for b := 0; b < NUM_OF_BLOCKS; b += 1 {
		...
		proposer, err := app.FuryaKeeper.GetFuryaValidator(ctx, proposerAddr)
		require.NoError(t, err)
		...
		app.FuryaKeeper.CompleteRedelegations(ctx)
		...
		res, stop := furya.RunAllInvariants(ctx, app.FuryaKeeper)
		if stop {
			panic(res)
		}
	}
	...
	state := app.FuryaKeeper.ExportGenesis(ctx)
	file, _ := os.Create("./benchmark_genesis.json")
	defer file.Close()
	file.Write(app.AppCodec().MustMarshalJSON(state))
}

func delegateOperation(ctx sdk.Context, app *test_helpers.App, r *rand.Rand, assets []types.FuryaAsset, vals []sdk.AccAddress, dels []sdk.AccAddress) {
	...
	app.FuryaKeeper.Delegate(ctx, delAddr, val, coins)
...
func redelegateOperation(ctx sdk.Context, app *test_helpers.App, r *rand.Rand, assets []types.FuryaAsset, vals []sdk.AccAddress, dels []sdk.AccAddress) {
	...
	srcValidator, _ := app.FuryaKeeper.GetFuryaValidator(ctx, srcValAddr)
	asset, _ := app.FuryaKeeper.GetAssetByDenom(ctx, delegation.Denom)

	if app.FuryaKeeper.HasRedelegation(ctx, delAddr, srcValAddr, asset.Denom) {
		return
	}
	...
	dstValidator, _ := app.FuryaKeeper.GetFuryaValidator(ctx, dstValAddr)

	delegation, found := app.FuryaKeeper.GetDelegation(ctx, delAddr, srcValidator, asset.Denom)
	if !found {
		return
	}
	...
	_, err := app.FuryaKeeper.Redelegate(ctx, delAddr, srcValidator, dstValidator, sdk.NewCoin(delegation.Denom, amountToRedelegate))
	if err != nil {
		panic(err)
	}
}
...
func undelegateOperation(ctx sdk.Context, app *test_helpers.App, r *rand.Rand) {
	...
	validator, _ := app.FuryaKeeper.GetFuryaValidator(ctx, valAddr)
	asset, _ := app.FuryaKeeper.GetAssetByDenom(ctx, delegation.Denom)

	delegation, found := app.FuryaKeeper.GetDelegation(ctx, delAddr, validator, asset.Denom)
	if !found {
		return
	}
	...
	app.FuryaKeeper.Undelegate(ctx, delAddr, validator, sdk.NewCoin(asset.Denom, amountToUndelegate))
}
...
func claimRewardsOperation(ctx sdk.Context, app *test_helpers.App, r *rand.Rand) {
	...
	delegation, found := app.FuryaKeeper.GetDelegation(ctx, delAddr, validator, delegation.Denom)
	if !found {
		return
	}

	_, err := app.FuryaKeeper.ClaimDelegationRewards(ctx, delAddr, validator, delegation.Denom)
	if err != nil {
		panic(err)
	}
}
