...
// Delegate is the entry point for delegators to delegate furya assets to validators
...
func (k Keeper) Delegate(ctx sdk.Context, delAddr sdk.AccAddress, validator types.FuryaValidator, coin sdk.Coin) (*sdk.Dec, error) {
	// Check if asset is whitelisted as an furya asset
	asset, found := k.GetAssetByDenom(ctx, coin.Denom)
	if !found {
		return nil, status.Errorf(codes.NotFound, "asset with denom: %s does not exist in furya whitelist", coin.Denom)
	}

	// Check and send delegated tokens into the furya module address
...
func (k Keeper) Redelegate(ctx sdk.Context, delAddr sdk.AccAddress, srcVal types.FuryaValidator, dstVal types.FuryaValidator, coin sdk.Coin) (*time.Time, error) {
...
func (k Keeper) Undelegate(ctx sdk.Context, delAddr sdk.AccAddress, validator types.FuryaValidator, coin sdk.Coin) (*time.Time, error) {
	asset, found := k.GetAssetByDenom(ctx, coin.Denom)
	if !found {
		return nil, status.Errorf(codes.NotFound, "Asset with denom: %s does not exist", coin.Denom)
	}
	...
	// Remove tokens and shares from the furya asset
...
func (k Keeper) GetDelegation(ctx sdk.Context, delAddr sdk.AccAddress, validator types.FuryaValidator, denom string) (d types.Delegation, found bool) {
...
func (k Keeper) SetValidator(ctx sdk.Context, val types.FuryaValidator) {
	store := ctx.KVStore(k.storeKey)
	key := types.GetFuryaValidatorInfoKey(val.GetOperator())
	vb := k.cdc.MustMarshal(val.FuryaValidatorInfo)
	store.Set(key, vb)
}

func (k Keeper) SetValidatorInfo(ctx sdk.Context, valAddr sdk.ValAddress, val types.FuryaValidatorInfo) {
	store := ctx.KVStore(k.storeKey)
...
func (k Keeper) ValidateDelegatedAmount(delegation types.Delegation, coin sdk.Coin, val types.FuryaValidator, asset types.FuryaAsset) (shares sdk.Dec, err error) {
...
func (k Keeper) upsertDelegationWithNewTokens(ctx sdk.Context, delAddr sdk.AccAddress, validator types.FuryaValidator, coin sdk.Coin, asset types.FuryaAsset) (types.Delegation, sdk.Dec) {
...
func (k Keeper) reduceDelegationShares(ctx sdk.Context, delAddr sdk.AccAddress, validator types.FuryaValidator, coin sdk.Coin, shares sdk.Dec, delegation types.Delegation) {
...
func (k Keeper) updateValidatorShares(ctx sdk.Context, validator types.FuryaValidator, delegationShares sdk.DecCoins, validatorShares sdk.DecCoins, isAdd bool) {
	if isAdd {
...
// GetFuryaBondedAmount returns the total amount of bonded native tokens that are not in the
// unbonding pool
func (k Keeper) GetFuryaBondedAmount(ctx sdk.Context, delegator sdk.AccAddress) math.Int {
	bonded := sdk.ZeroDec()
