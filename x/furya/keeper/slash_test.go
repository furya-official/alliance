...
func TestSlashingEvent(t *testing.T) {
	...
	addrs := test_helpers.AddTestAddrsIncremental(app, ctx, 4, sdk.NewCoins(
		sdk.NewCoin(FURYA_TOKEN_DENOM, sdk.NewInt(20_000_000)),
		sdk.NewCoin(FURYA_2_TOKEN_DENOM, sdk.NewInt(20_000_000)),
	))
	...
	_, err = app.FuryaKeeper.Delegate(ctx, user1, val1, sdk.NewCoin(FURYA_TOKEN_DENOM, sdk.NewInt(10_000_000)))
	require.NoError(t, err)
	...
	asset1, _ := app.FuryaKeeper.GetAssetByDenom(ctx, FURYA_TOKEN_DENOM)
	tokens := val1.TotalTokensWithAsset(asset1).TruncateInt()
	require.Equal(t, sdk.NewInt(20_000_000), tokens)
	asset2, _ := app.FuryaKeeper.GetAssetByDenom(ctx, FURYA_2_TOKEN_DENOM)
...
func TestSlashingAfterRedelegation(t *testing.T) {
	...
	_, err = app.FuryaKeeper.Redelegate(ctx, user1, val1, val2, sdk.NewCoin(FURYA_TOKEN_DENOM, sdk.NewInt(10_000_000)))
	require.NoError(t, err)
	...
	delegation, _ := app.FuryaKeeper.GetDelegation(ctx, user1, val2, FURYA_TOKEN_DENOM)
	...
	// Now we slash val 1
	app.FuryaKeeper.GetFuryaValidator(ctx, valAddr1)
	app.SlashingKeeper.Slash(ctx, valConAddr1, slashFraction, valPower1, 1)
...
func TestSlashingAfterUndelegation(t *testing.T) {
	...
	_, err = app.FuryaKeeper.Undelegate(ctx, user1, val1, sdk.NewCoin(FURYA_TOKEN_DENOM, sdk.NewInt(10_000_000)))
	require.NoError(t, err)

	// Expect to have undelegation index saved
	undelegationIndexIter := app.FuryaKeeper.IterateUndelegationsBySrcValidator(ctx, valAddr1)
	...
	undelegationsIter := app.FuryaKeeper.IterateUndelegationsByCompletionTime(ctx, ctx.BlockTime().Add(app.StakingKeeper.UnbondingTime(ctx)).Add(time.Second))
	...
	// Expect that delegation stayed the same
	undelegationsIter = app.FuryaKeeper.IterateUndelegationsByCompletionTime(ctx, ctx.BlockTime())
	require.True(t, undelegationsIter.Valid())
