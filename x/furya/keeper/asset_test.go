...
func TestRebalancingAfterRewardsRateChange(t *testing.T) {
	...
	app.FuryaKeeper.InitGenesis(ctx, &types.GenesisState{
		...
		Assets: []types.FuryaAsset{
			types.NewFuryaAsset(FURYA_TOKEN_DENOM, sdk.NewDec(2), sdk.ZeroDec(), startTime),
			types.NewFuryaAsset(FURYA_2_TOKEN_DENOM, sdk.NewDec(10), sdk.ZeroDec(), startTime),
		},
	})
	...
	// Expecting voting power for the furya module
	...
	err = app.FuryaKeeper.UpdateFuryaAsset(ctx, types.FuryaAsset{
	...
	iter := app.FuryaKeeper.IterateWeightChangeSnapshot(ctx, FURYA_TOKEN_DENOM, val.GetOperator(), 0)
...
func TestRebalancingWithUnbondedValidator(t *testing.T) {
	...
	addrs := test_helpers.AddTestAddrsIncremental(app, ctx, 5, sdk.NewCoins(
		...
		sdk.NewCoin(FURYA_TOKEN_DENOM, sdk.NewInt(50_000_000)),
		sdk.NewCoin(FURYA_2_TOKEN_DENOM, sdk.NewInt(50_000_000)),
	))
	pks := test_helpers.CreateTestPubKeys(2)
	...
	_, err = app.FuryaKeeper.Delegate(ctx, user1, val1, sdk.NewCoin(FURYA_TOKEN_DENOM, sdk.NewInt(20_000_000)))
	require.NoError(t, err)
	_, err = app.FuryaKeeper.Delegate(ctx, user1, val2, sdk.NewCoin(FURYA_TOKEN_DENOM, sdk.NewInt(10_000_000)))
	require.NoError(t, err)
	_, err = app.FuryaKeeper.Delegate(ctx, user2, val1, sdk.NewCoin(FURYA_TOKEN_DENOM, sdk.NewInt(10_000_000)))
	require.NoError(t, err)
	_, err = app.FuryaKeeper.Delegate(ctx, user2, val2, sdk.NewCoin(FURYA_TOKEN_DENOM, sdk.NewInt(10_000_000)))
	require.NoError(t, err)

	_, err = app.FuryaKeeper.Delegate(ctx, user1, val1, sdk.NewCoin(FURYA_2_TOKEN_DENOM, sdk.NewInt(10_000_000)))
	require.NoError(t, err)
	_, err = app.FuryaKeeper.Delegate(ctx, user1, val2, sdk.NewCoin(FURYA_2_TOKEN_DENOM, sdk.NewInt(10_000_000)))
	require.NoError(t, err)
	_, err = app.FuryaKeeper.Delegate(ctx, user2, val1, sdk.NewCoin(FURYA_2_TOKEN_DENOM, sdk.NewInt(10_000_000)))
	require.NoError(t, err)
	_, err = app.FuryaKeeper.Delegate(ctx, user2, val2, sdk.NewCoin(FURYA_2_TOKEN_DENOM, sdk.NewInt(10_000_000)))
	require.NoError(t, err)
	require.NoError(t, err)
	...
	app.FuryaKeeper.GetFuryaValidator(ctx, valAddr2)
...
func TestRebalancingWithDelayedRewardsStartTime(t *testing.T) {
	...
	app.FuryaKeeper.InitGenesis(ctx, &types.GenesisState{
		Params: types.DefaultParams(),
		Assets: []types.FuryaAsset{
			types.NewFuryaAsset(FURYA_TOKEN_DENOM, sdk.MustNewDecFromStr("0.5"), sdk.MustNewDecFromStr("0.1"), startTime.Add(time.Hour*24)),
			types.NewFuryaAsset(FURYA_2_TOKEN_DENOM, sdk.MustNewDecFromStr("0.2"), sdk.MustNewDecFromStr("0.1"), startTime.Add(time.Hour*24*2)),
		},
	})
	...
	// Expect that rewards rates are updated only for furya 1
	...
	// Expect that rewards rates are updated all furyas
...
func TestConsumingRebalancingEvent(t *testing.T) {
	...
	app.FuryaKeeper.QueueAssetRebalanceEvent(ctx)
	store := ctx.KVStore(app.FuryaKeeper.StoreKey())
	key := types.AssetRebalanceQueueKey
	b := store.Get(key)
	require.NotNil(t, b)

	require.True(t, app.FuryaKeeper.ConsumeAssetRebalanceEvent(ctx))
	b = store.Get(key)
	require.Nil(t, b)

	require.False(t, app.FuryaKeeper.ConsumeAssetRebalanceEvent(ctx))
}
...
func TestRewardWeightDecay(t *testing.T) {
	...
	app.FuryaKeeper.RewardWeightChangeHook(ctx, assets)
	updatedAsset, _ := app.FuryaKeeper.GetAssetByDenom(ctx, FURYA_TOKEN_DENOM)
	...
	// Updating the furya asset through proposal should queue another decay event
	app.FuryaKeeper.UpdateFurya(ctx, &types.MsgUpdateFuryaProposal{
		Title:                "",
		Description:          "",
		Denom:                FURYA_TOKEN_DENOM,
		RewardWeight:         sdk.MustNewDecFromStr("0.5"),
	...
	// Updating furya asset again with a non-zero decay
	app.FuryaKeeper.UpdateFurya(ctx, &types.MsgUpdateFuryaProposal{
		Title:                "",
		Description:          "",
		Denom:                FURYA_TOKEN_DENOM,
	...
	// Updating furya asset again with a non-zero decay
	err = app.FuryaKeeper.UpdateFurya(ctx, &types.MsgUpdateFuryaProposal{
		Title:                "",
		Description:          "",
		Denom:                FURYA_2_TOKEN_DENOM,
		RewardWeight:         sdk.MustNewDecFromStr("0.5"),
...
func TestRewardWeightDecayOverTime(t *testing.T) {
	...
	totalDecayTime := (time.Hour * 24 * 10) - app.FuryaKeeper.RewardDelayTime(ctx)
	intervals := uint64(totalDecayTime / decayInterval)
	asset, _ = app.FuryaKeeper.GetAssetByDenom(ctx, FURYA_TOKEN_DENOM)
	require.Equal(t, startTime.Add(app.FuryaKeeper.RewardDelayTime(ctx)).Add(decayInterval*time.Duration(intervals)), asset.LastRewardChangeTime)
...
func TestClaimTakeRate(t *testing.T) {
	...
	app.FuryaKeeper.InitGenesis(ctx, &types.GenesisState{
		...
		Assets: []types.FuryaAsset{
			types.NewFuryaAsset(FURYA_TOKEN_DENOM, sdk.NewDec(2), sdk.MustNewDecFromStr("0.5"), startTime),
	...
	addrs := test_helpers.AddTestAddrsIncremental(app, ctx, 1, sdk.NewCoins(
		...
		sdk.NewCoin(FURYA_2_TOKEN_DENOM, sdk.NewInt(1000_000_000)),
	))
	user1 := addrs[0]

	app.FuryaKeeper.Delegate(ctx, user1, val1, sdk.NewCoin(FURYA_TOKEN_DENOM, sdk.NewInt(1000_000_000)))
	app.FuryaKeeper.Delegate(ctx, user1, val1, sdk.NewCoin(FURYA_2_TOKEN_DENOM, sdk.NewInt(1000_000_000)))
	...
	coinsClaimed, _ := app.FuryaKeeper.DeductAssetsHook(ctx, assets)
	...
	require.Equal(t, expectedAmount.TruncateInt(), coins.AmountOf(FURYA_TOKEN_DENOM))

	lastUpdate := app.FuryaKeeper.LastRewardClaimTime(ctx)
	require.Equal(t, startTime.Add(takeRateInterval), lastUpdate)

	asset, found := app.FuryaKeeper.GetAssetByDenom(ctx, FURYA_TOKEN_DENOM)
	require.True(t, found)
	...
	require.Equal(t,
		sdk.NewDecFromInt(coinsClaimed.AmountOf(FURYA_TOKEN_DENOM)),
		rewards.AmountOf(FURYA_TOKEN_DENOM).Add(community.AmountOf(FURYA_TOKEN_DENOM)),
	)
}
...
func TestClaimTakeRateToZero(t *testing.T) {
	...
	ctx = ctx.WithBlockHeight(1)
	takeRateInterval := time.Minute * 5
	asset := types.NewFuryaAsset(FURYA_TOKEN_DENOM, sdk.NewDec(2), sdk.MustNewDecFromStr("0.8"), startTime)
	app.FuryaKeeper.InitGenesis(ctx, &types.GenesisState{
		Params: types.Params{
			RewardDelayTime:       time.Minute * 60,
			TakeRateClaimInterval: takeRateInterval,
	...
	for i := 0; i < 1000; i++ {
		...
		_, err = app.FuryaKeeper.DeductAssetsHook(ctx, assets)
		require.NoError(t, err)
	}
