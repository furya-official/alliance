...
import (
	...
	kaijutypes "github.com/furya-official/kaiju/x/kaiju/types"
...
Example:
	kaijud testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.5.20
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "kaijud", "Home directory of the node's daemon configuration")
...
Example:
	kaijud testnet --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initGenFiles(
	...
	// KAIJU
	var kaijuGenState kaijutypes.GenesisState
	clientCtx.Codec.MustUnmarshalJSON(appGenState[kaijutypes.ModuleName], &kaijuGenState)
	kaijuParams := kaijuGenState.GetParams()
	kaijuParams.TakeRateClaimInterval = time.Second * 30
	kaijuParams.RewardDelayTime = time.Second * 30
	kaijuGenState.Params = kaijuParams
	appGenState[kaijutypes.ModuleName] = clientCtx.Codec.MustMarshalJSON(&kaijuGenState)
