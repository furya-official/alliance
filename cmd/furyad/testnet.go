...
Example:
	furyad testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.5.20
	`,
...
Example:
	furyad testnet --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initGenFiles(
	...
	// FURYA
	var furyaGenState furyatypes.GenesisState
	clientCtx.Codec.MustUnmarshalJSON(appGenState[furyatypes.ModuleName], &furyaGenState)
	furyaParams := furyaGenState.GetParams()
	furyaParams.TakeRateClaimInterval = time.Second * 30
	furyaParams.RewardDelayTime = time.Second * 30
	furyaGenState.Params = furyaParams
	appGenState[furyatypes.ModuleName] = clientCtx.Codec.MustMarshalJSON(&furyaGenState)
